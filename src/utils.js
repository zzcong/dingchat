...
const TokenTable = require('./tokenModel.js')
const conf = require('./config.json')

// 从环境变量中获取到钉钉的相关配置
const DING_APP_KEY = conf.DING_APP_KEY || '';
...
async function getAccessToken() {
  ...
  const item = await TokenTable.find().where().sort({ expiredAt: -1 }).findOne();
  const now = Date.now();
  ...
  // 将 token 存入数据库
  await TokenTable.create({ token, expiredAt });
...
function handleError(error) {
  ...
  if (error.response) {
    ...
    if (status === 401) {
      // 401 代表 OpenAI 认证失败了
      errorMessage = '你没有配置正确的 OpenAI API Key，请进入 AirCode 中配置正确的环境变量。'
    ...
    errorMessage = 'OpenAI 服务器没有响应，可以前往 https://status.openai.com/ 查看其服务状态。';
